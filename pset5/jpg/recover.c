 ...
 */
 ...
 int main(void)
 {
    ...
    uint8_t* check_bytes = malloc(BLOCK_SIZE);
    
    // finds the start of the first jpeg
    do
    {
        fread(check_bytes, BLOCK_SIZE, 1, infile);
 ...
 **/
 ...
 void write_new(FILE* infile, int* filenum, char* filename)
 {
    ...
    *filenum = *filenum + 1;
    ...
    do
    {
        char* write_bytes = malloc(BLOCK_SIZE);
        fseek(infile, -512, SEEK_CUR);
        fread(write_bytes, BLOCK_SIZE, 1, infile);
        
        fwrite(write_bytes, BLOCK_SIZE, 1, outfile);
